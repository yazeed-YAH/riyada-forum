...
export default function Home() {
  ...
  const formatDateOnly = (dateString: string) => {
    const date = new Date(dateString)
    return toEnglishNumbers(date.toLocaleDateString('ar-SA', { year: 'numeric', month: 'long', day: 'numeric', numberingSystem: 'latn' }))
  }

  const getDayName = (dateString: string) => {
    const date = new Date(dateString)
    return toEnglishNumbers(date.toLocaleDateString('ar-SA', { weekday: 'long' }))
  }
            ...
            <nav className="container py-6 flex flex-col gap-4">
              ...
              {member || admin ? (
                ...
                <Link href="/login" onClick={() => setMobileMenuOpen(false)}
                  className="text-sm font-medium" style={{ color: '#2d1f26' }}>
                  تسجيل الدخول
                </Link>
              )}
            </nav>
